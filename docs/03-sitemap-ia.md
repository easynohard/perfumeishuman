...
- 로그인은 **화면이 아니라 순간**이다 — 쓰기·추천을 시도하는 순간과 MY 탭의 로그인 버튼에서만 바텀시트로 나타난다(N2). 열람 경로에는 로그인 벽이 없다. **탈퇴도 같은 원칙으로 화면이 아니라 MY 안의 확인 모달이다**(F3).
...
| 로그인 | 독립 페이지·탭 없음 | 쓰기·추천 시도 순간 + MY 탭의 로그인 버튼 — 모두 바텀시트로 등장. **약관·처리방침 동의 고지를 버튼 영역에 포함**(F1) |
...
| 로그인 게이트 | (바텀시트) | 독립 페이지 없음 — 어느 화면에서든 오버레이. 소셜 1탭, 완료 후 직전 행동 복귀. 진입: 쓰기·추천 시도 + MY 탭 로그인 버튼. 콜백 라우트(`/auth/callback`)만 존재. **약관·처리방침 동의 고지 포함** | C1·M4·F1 |
...
| FL3 | **쓰기 전환** | 향수 상세 [시향기 쓰기] → (게스트면) 로그인 바텀시트 → 작성 폼(임시저장 복원) → 등록 → 시향기 상세 + 피드 노출 | 서비스의 심장. 쓰기 탭 직접 진입 시 향수 선택부터 시작. 로그인 전 작성 내용 보존(N2·N3) |
