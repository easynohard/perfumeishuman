...
> 개정: 2026-09-13 — `/write`를 게스트 열람 화면에서 멤버 전용 화면으로 재분류. "시향기 쓰기"를 누르는 순간 로그인 게이트를 먼저 거친다(2026-08-19 결정을 되돌림). 근거: [ADR 0014](./adr/0014-login-before-write-entry.md)
...
- 로그인은 **화면이 아니라 순간**이다 — **"시향기 쓰기"**·추천을 시도하는 순간과 MY 탭의 로그인 버튼에서만 바텀시트로 나타난다(N2). 열람 경로에는 로그인 벽이 없다. `/write`는 로그인해야 진입할 수 있는 화면이다 — "시향기 쓰기"를 누르는 순간 게이트가 먼저 뜨고, 로그인 완료 후 빈 작성 화면으로 이동한다(ADR 0014). **탈퇴도 같은 원칙으로 화면이 아니라 MY 안의 확인 모달이다**(F3).
...
flowchart LR
  ...
  subgraph MEMBER["멤버 — 로그인은 이 순간에만"]
    ...
    WRITE["시향기 작성 폼<br>/write · 로그인해야 진입 가능"]
  ...
  PERFUME -->|시향기 쓰기 - 멤버일 때| WRITE
  PERFUME -.->|시향기 쓰기 - 게스트일 때| GATE
  GATE -->|로그인 완료 - 쓰기였을 때| WRITE
  GATE -->|로그인 완료 - 추천이었을 때| REVIEW
...
- **`/write`는 멤버 전용 화면이다**(ADR 0014) — 게스트가 "시향기 쓰기"를 누르면 로그인 게이트를 먼저 거치고, 로그인 완료 후 선택했던 향수가 채워진 빈 작성 화면으로 진입한다. 멤버는 게이트 없이 곧장 진입한다.
...
| 로그인 | 독립 페이지·탭 없음 | **"시향기 쓰기"**·추천 시도 순간 + MY 탭의 로그인 버튼 — 모두 바텀시트로 등장. `/write` 진입 자체가 로그인 필요(ADR 0014). **약관·처리방침 동의 고지를 버튼 영역에 포함**(F1) |
...
| 화면 | 경로 | 핵심 내용 | 근거 |
|---|---|---|---|
| 시향기 작성 폼 | `/write` | 한 페이지 스크롤 폼: 자유 서술 100자(상단) + 태그 질문 2종(하단), 질문 카드 버튼, 자동 임시저장. **로그인해야 진입 가능**(ADR 0014) — 게스트가 "시향기 쓰기"를 누르면 로그인 게이트를 먼저 거친 뒤 빈 작성 화면으로 진입한다. 수정 모드(본인 글만)는 같은 폼에 기존 값 채움 | D1·D2·M5 |
...
| FL3 | **쓰기 전환** | 향수 상세 [시향기 쓰기] → (게스트면) 로그인 바텀시트 → 작성 폼(향수 선택된 채 시작, 임시저장 복원) → [등록] → 시향기 상세 + 피드 노출 | 서비스의 심장. 쓰기 탭 직접 진입 시 향수 선택부터 시작. 로그인은 쓰기를 시작하는 시점에 요구된다(N2·N3, ADR 0014) |
...
3. **로그인은 독립 페이지 없음** — 전 화면 바텀시트. **"시향기 쓰기"**·추천 시도 순간뿐 아니라 MY 탭의 로그인 버튼에서도 동일하게 바텀시트로 진행. 소셜 콜백용 라우트(`/auth/callback`)만 둔다.
...
### 추가 결정 1개 (2026-09-13 사용자 승인) ✅

8. **`/write`는 멤버 전용 화면으로 재분류** — 게스트 열람 11개 중 하나였던 작성 폼을 멤버 전용으로 옮긴다(게스트 열람 10 + 게이트 1, 멤버 전용 2). "시향기 쓰기"를 누르는 순간 로그인 게이트가 먼저 뜨고, 로그인 완료 후 빈 작성 화면으로 이동한다 — 2026-08-16에 없던 새 결정이 아니라 **2026-08-19에 바뀌었던 정책을 되돌리는 것**이다 ([ADR 0014](./adr/0014-login-before-write-entry.md)).
